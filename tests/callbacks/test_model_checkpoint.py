...
def test_model_checkpoint_format_checkpoint_name(tmpdir):
    ...
    # no filepath set
    ckpt_name = ModelCheckpoint(monitor='val_loss', filepath=None).format_checkpoint_name(3, 4, {})
    assert ckpt_name == 'epoch=3-step=4.ckpt'
    ckpt_name = ModelCheckpoint(monitor='val_loss', filepath='').format_checkpoint_name(5, 4, {})
    assert ckpt_name == 'epoch=5-step=4.ckpt'
    # CWD
    ckpt_name = ModelCheckpoint(monitor='val_loss', filepath='.').format_checkpoint_name(3, 4, {})
    assert Path(ckpt_name) == Path('.') / 'epoch=3-step=4.ckpt'
    # dir does not exist so it is used as filename
    filepath = tmpdir / 'dir'
    ckpt_name = ModelCheckpoint(monitor='val_loss', filepath=filepath, prefix='test').format_checkpoint_name(3, 4, {})
    ...
    ckpt_name = ModelCheckpoint(
        monitor='val_loss', filepath=tmpdir / 'name', prefix='test'
    ).format_checkpoint_name(3, 4, {}, ver=3)
...
def test_model_checkpoint_save_last(tmpdir):
    ...
    model_checkpoint = ModelCheckpoint(monitor='val_loss', filepath=tmpdir / '{step}', save_top_k=-1, save_last=True)
    trainer = Trainer(
        default_root_dir=tmpdir,
        early_stop_callback=False,
        checkpoint_callback=model_checkpoint,
